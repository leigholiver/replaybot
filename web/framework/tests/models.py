    ...
    def run(self):
        ...
        if skip_message == "":
            ...
            try:
                ...
                # batch get items by id
                self.header("batch get objects")
                test_model_ids = []
                for i in range(0,5):
                    tmp = random.choice(test_models).id
                    if tmp not in test_model_ids:
                        test_model_ids.append(tmp)
                results = test_model.batch_get(test_model_ids)
                self.record(result, "len(results) == " + str(len(test_model_ids)), "len(results) == " + str(len(results)))

                # batch update objects
                self.header("batch update objects")
                for mod in test_models:
                    mod.name = "updated!"
                test_model.batch_save(test_models)
                tmp = test_model.get(random.choice(test_models).id)
                result = tmp.name == "updated!"
