import os, pkgutil, inspect, json, re, yaml
from framework.lamb.model import model
from git import Repo

class env_util():
    environments_dir   = "environment/deployments/"
    defaults_path      = "environment/env.yml"
    defaults_path_json = "environment/env.json"
    ...
    exclude_fields = [
        ...
        "git_base_path"     # path to the root of the git repo
    ]
    ...
    def get_full(self, ignore_local=False):
        envs = self.get_environments()
        if envs == []:
            print("No environment definitions found")
            return False

        if os.getenv('LAMB_ENV') == "local" and not ignore_local:
    ...
    def get_tf_vars(self):
        e = self.get_full(ignore_local=True)
        if e == False:
            return False
        ...
        for e in envs:
            ...
            for f in envs:
                if 'filename' in e.keys() and 'filename' in f.keys() and e['filename'] != f['filename']:
                    ...
                    if e['branch_specifier'] == f['branch_specifier']:
                        self.add_error(errors, "warning: duplicate branch specifier " + e['branch_specifier'] + " - " + e['filename'] + " and " + f['filename'])
    ...
    def process_buckets(self, e):
        if 'buckets' in e.keys():
            for bucket in e['buckets']:
                bucket['domain_name'] = e['domain_name']
                if "subdomain" in bucket.keys():
                    bucket['domain_name'] = "%s.%s" % (bucket['subdomain'], e['domain_name'])
                    bucket.pop('subdomain', None)
    ...
    def process_secrets(self, e):
        if isinstance(e, dict):
            for dict_key in e.keys():
                e[dict_key] = self.process_secrets(e[dict_key])
            return e
        elif isinstance(e, list):
            items = []
            for item in e:
                items.append(self.process_secrets(item))
            return items
        elif isinstance(e, str):
            match = re.search("{{ secrets\.(.*) }}", e)
            if match:
                e = e.replace(match.group(0), str(self.get_secret(match.group(1))))
        return e
    ...
    def get_from_git(self, envs):
        defaults = self.get_defaults()
        repo = Repo(os.path.abspath(defaults['git_base_path']) if 'git_base_path' in defaults.keys() else os.getcwd())
        try:
    ...
    def combine_with_defaults(self, data):
        defaults = self.get_defaults()
        out = defaults.copy()
        out = self.dict_merge_recursive(out, data)
        return out

    def dict_merge_recursive(self, out, data):
        for key in data.keys():
            ...
            # merge dicts
                ...
                out[key] = self.dict_merge_recursive(out[key], data[key])
            else:
                out[key] = data[key]
        return out
    ...
    def get_defaults(self):
        try:
            return yaml.load(open(self.defaults_path), Loader=yaml.FullLoader)
        except:
            return json.load(open(self.defaults_path_json))
    ...
    def get_environments(self):
        ...
        for file in files:
            ...
            if file.endswith(".yml") or file.endswith(".yaml"):
                try:
                    with open(self.environments_dir + file) as yaml_file:
                        data = yaml.load(yaml_file, Loader=yaml.FullLoader)
                except Exception as e:
                    print(e)
                    print("skipping " + file + " - error parsing yaml")

            if data and "env_name" in data.keys() and "branch_specifier" in data.keys():
                data['filename'] = file
