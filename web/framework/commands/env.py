...
class env(command):
    ...
    def __init__(self):
        ...
        self.full   = self.e_util.get_full(ignore_local=True)
    ...
    def run(self, data):
        ...
        if len(data) == 1 and data[0] == "create":
            ...
            return self.create_environment_yaml(data)
    ...
    def get_environment_data(self, data):
    ...
    def create_environment_json(self, data):
        env = self.get_environment_data(data)
        if not env:
            return False
        return self.write_environment_json(env)

    def write_environment_json(self, data):
        output_file = self.output_dir + data['env_name'] + ".json"
    ...
    def create_environment_yaml(self, data):
        env = self.get_environment_data(data)
        if not env:
            return False
        return self.write_environment_yaml(env)

    def write_environment_yaml(self, data):
        output_file = self.output_dir + data['env_name'] + ".yml"
        if os.path.isfile(output_file):
            print("error: environment file already exists")
            return False
        try:
